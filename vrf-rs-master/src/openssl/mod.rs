...
impl ECVRF {
    ...
    pub fn sign_counter_init(&mut self, secret_key: &[u8]) -> Result<Vec<u8>, Error> {
        let secret_key_bn = BigNum::from_slice(secret_key)?;

        // Derive the public key point from the secret key
        let public_key_point = self.derive_public_key_point(&secret_key_bn)?;
        
        // Create an ECDSA key from the secret key and the derived public key
        let ecdsa_key = openssl::ec::EcKey::from_private_components(
            &self.group,
            &secret_key_bn,
            &public_key_point
        )?;

        // Convert EcKey to PKey
        let pkey = PKey::from_ec_key(ecdsa_key)?;

        // Create a signer for signing the message
        let mut signer = Signer::new(self.hasher, &pkey)?;
        
        // Hard-coded message "counter_init"
        let message = b"counter_init";
    ...
    pub fn validate_counter_init_signature(
        &mut self,
        public_key: &[u8],
        signature: &[u8],
    ) -> Result<bool, Error> {
        let public_key_point = EcPoint::from_bytes(&self.group, public_key, &mut self.bn_ctx)?;
        
        // Create an ECDSA key from the public key point
        let ecdsa_key = openssl::ec::EcKey::from_public_key(&self.group, &public_key_point)?;
        
        // Convert EcKey to PKey
        let pkey = PKey::from_ec_key(ecdsa_key)?;
        
        // Create a verifier for verifying the signature
        let mut verifier = Verifier::new(self.hasher, &pkey)?;
